...
int ErrorsCheck::IsExpression(int i, FILE* ef)
{
    ...
    for (; (Data->LexTable[nom].type != LSeparator && (Data->LexTable[nom].type != LStart)); nom++)
    {
...
int ErrorsCheck::ErrorChecking()
{
    ...
    if (Data->LexTable[i++].type != LVariable)
    {
        Err_num++;
        fprintf(ef, "line %d: \t'%s' expected!\n", Data->LexTable[i].line, Data->LexName[LVariable].name.c_str());
    }
    ...
    switch (Balans(0, LEOF, LStart, LStop))
    {
    case 1:
        Err_num++;
        fprintf(ef, "line %d: \tToo much '%s'!\n", Data->LexTable[j].line, Data->LexName[LStop].name.c_str());
        break;
    case 2:
        Err_num++;
        fprintf(ef, "line %d: \t'%s' expected!\n", Data->LexTable[j].line, Data->LexName[LStop].name.c_str());
    ...
    switch (Balans(0, LEOF, LDo, LWhile))
    {
    case 1:
        Err_num++;
        fprintf(ef, "line %d: \tToo much '%s'!\n", Data->LexTable[j].line, Data->LexName[LWhile].name.c_str());
        break;
    case 2:
        Err_num++;
        fprintf(ef, "line %d: \t'%s' expected!\n", Data->LexTable[j].line, Data->LexName[LWhile].name.c_str());
    ...
    if (Data->LexTable[Data->LexTable.size() - 2].type != LStop)
    {
        Err_num++;
        fprintf(ef, "line %d: \t'%s' expected!\n", Data->LexTable[Data->LexTable.size() - 2].line, Data->LexName[LStop].name.c_str());
    }
    ...
    for (j = 0;; j++)
    {
        ...
        if (Data->LexTable[j].type == LWhile || Data->LexTable[j].type == LIf)
        {
            if (Data->LexTable[j + 1].type != LLBraket)
            {
                Err_num++;
                fprintf(ef, "line %d: \t'(' expected after '%s'!\n", Data->LexTable[j + 1].line, Data->LexName[Data->LexTable[j].type].name.c_str());
            }
